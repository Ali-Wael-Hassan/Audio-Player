...
void PlayerGUI::initializeControls()
{
    ...
    //forwardButton.setButtonText(juce::String::fromUTF8("\xE2\x8F\xA9")); 
    //forwardButton.setColour(juce::TextButton::buttonColourId, juce::Colour::fromRGB(70, 130, 180));
    //forwardButton.setColour(juce::TextButton::textColourOffId, juce::Colours::white);
    //backwardButton
    addAndMakeVisible(backwardButton);
    backwardButton.addListener(this);
    //backwardButton.setButtonText(juce::String::fromUTF8("\xE2\x8F\xAA")); 
    //backwardButton.setColour(juce::TextButton::buttonColourId, juce::Colour::fromRGB(70, 130, 180));
    //backwardButton.setColour(juce::TextButton::textColourOffId, juce::Colours::white);
    ...
    speedSlider.setRange(0.5, 2.0, 0.25);
...
void PlayerGUI::handleAsyncUpdate()
{
    ...
    if (!control->isLooping() && currentPlaylistIndex >= 0)
    {
        auto songs = control->getPlaylistManager().getSongs("front");
        if (currentPlaylistIndex + 1 < songs.size())
        {
